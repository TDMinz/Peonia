...
import { staffAdminApi, type ShopOrderItem } from '../services/adminStaff';
...
export default function StaffShopOrdersPage() {
  const [items, setItems] = useState<ShopOrderItem[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [message, setMessage] = useState('');
  const [error, setError] = useState('');
  const [selected, setSelected] = useState<ShopOrderItem | null>(null);
  ...
  async function loadData() {
    ...
    setError('');
    try {
      const data = await staffAdminApi.shopOrders();
      setItems(data.data || []);
    } catch (err) {
  ...
  function openDetail(item: ShopOrderItem) {
    setSelected(item);
  ...
  async function handleSaveStatus() {
    ...
    setMessage('');
    try {
      const data = await staffAdminApi.updateShopOrderStatus(selected.order_code, status);
  ...
  return (
    <StaffLayout title="Quản lý đơn hàng" subtitle="Xem chi tiết và cập nhật trạng thái đơn hàng">
            ...
            <div className="grid gap-6 p-6 lg:grid-cols-[1fr_0.9fr]">
              ...
              <div className="space-y-4 rounded-[1.5rem] bg-[#f6f7fb] p-5">
                <label className="block">
                  <span className="mb-2 block text-sm font-medium text-foreground">Trạng thái đơn</span>
                  <select value={status} onChange={(e) => setStatus(e.target.value)} className="w-full rounded-2xl border border-[#e8edf3] bg-white px-4 py-3 text-sm outline-none">
                ...
                <div className="rounded-2xl border border-[#e8edf3] bg-white p-4 text-sm text-[#6f7b8b]">
                  ...
                  <div className="mt-2 flex justify-between"><span>Thanh toán</span><span className="font-medium text-foreground">{selected.payment_status}</span></div>
                </div>

                <button disabled={saving} onClick={handleSaveStatus} className="w-full rounded-full bg-[#111827] px-5 py-4 text-sm font-medium text-white transition hover:bg-black disabled:opacity-60">
                  {saving ? 'Đang cập nhật...' : 'Cập nhật trạng thái đơn'}
                </button>
              </div>
            </div>
          </div>
